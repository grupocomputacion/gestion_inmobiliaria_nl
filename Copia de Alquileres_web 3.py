...
def init_tablas():
    ddls = [
        ...
        """CREATE TABLE IF NOT EXISTS inmuebles (
            ...
            deposito_base BIGINT DEFAULT 0, estado TEXT DEFAULT 'Libre')""",
...
def generar_pdf_contrato(datos_u, datos_i, f_inicio, m_alq, m_dep, m_con, uso):
    ...
    texto = (
        ...
        f'Firmado en Córdoba, {hoy.day} de {meses_nom[hoy.month-1]} de {hoy.year}.'
    )
    ...
    if guardar:
        try:
            ...
            nid  = db_query(
                ...
                " VALUES (%s,%s,%s,%s,%s,1)",
                (int(uid), int(iid), fini, f_v, cl(ma)), commit=True)
            if nid:
                db_query("INSERT INTO deudas (id_contrato,concepto,monto_debe,pagado) VALUES (%s,'Alquiler',%s,0)", (nid, cl(ma)), commit=True)
                db_query("INSERT INTO deudas (id_contrato,concepto,monto_debe,pagado) VALUES (%s,'Depósito',%s,0)", (nid, cl(md)), commit=True)
                pdf_bytes = generar_pdf_contrato(s_u.to_dict(), s_i.to_dict(), fini, ma, md, mc, uso_sel)
                st.session_state["pdf_f"] = pdf_bytes
                st.session_state["id_f"]  = nid
                st.success(f"✅ Contrato #{nid} registrado.")
            else:
                st.error("La DB rechazó el registro.")
        except Exception as e:
            st.error(f"Error: {e}")
...
# ==============================================================================
    ...
    with t2:
    # ==============================================================================
    # SUB-MENU: UNIDADES (DENTRO DE CARGAS) - CORREGIDO
    # ==============================================================================
    #elif sel_sub == "Unidades":
        st.subheader("🏢 Gestión de Unidades")

        # --- 1. ALTA DE NUEVA UNIDAD (Expander para no molestar) ---
        with st.expander("➕ Dar de Alta Nueva Unidad"):
            with st.form("form_alta_u"):
                ...
                if df_bl is not None and not df_bl.empty:
                    # Mapeo de nombre a ID para el INSERT
                    ...
                    if st.form_submit_button("💾 Guardar Nueva Unidad"):
                        if f_tipo:
                            db_query("""
                                INSERT INTO inmuebles (id_bloque, tipo, precio_alquiler, costo_contrato, deposito_base, estado)
                                VALUES (%s, %s, %s, %s, %s, 'Libre')
                            """, (dic_bl[f_bl], f_tipo, f_alq, f_con, f_dep), commit=True)
        ...
        # Query con alias protegidos para evitar KeyErrors
        df_uni = db_query("""
            SELECT 
                i.id AS "id", 
                b.nombre AS "Inmueble", 
        ...
        if df_uni is not None and not df_uni.empty:
            ...
            with st.form("form_edit_unidad_cargas"):
                ...
                # Buscamos el índice actual del estado para el selectbox
                ...
                if btn_c1.form_submit_button("💾 Guardar Cambios"):
                    db_query("""
                        ...
                        WHERE id=%s
                    """, (e_tipo, e_alq, e_con, e_dep, e_est, int(row_u["id"])), commit=True)
                    st.success("✅ Cambios actualizados.")
                    st.rerun()
                    
                if btn_c2.form_submit_button("🗑️ Eliminar Unidad"):
                    db_query("DELETE FROM inmuebles WHERE id=%s", (int(row_u["id"]),), commit=True)
                    st.warning("🗑️ Unidad eliminada del sistema.")
                    st.rerun()
        else:
            st.info("ℹ️ No hay unidades cargadas en el sistema.")
    ...
    with t4:
        ...
        if df_cont is not None and not df_cont.empty:
            # Ahora sí, los nombres coinciden con los alias de la query arriba
            st.dataframe(
                df_cont[["ID_Contrato", "Inmueble", "Unidad", "Inquilino", "Inicio", "Fin", "Alquiler"]],
                use_container_width=True, 
                hide_index=True
            )
        else:
            st.info("ℹ️ No se encontraron contratos registrados.")
