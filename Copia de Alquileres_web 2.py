...
# --- FUNCIONES DE FORMATEO (DEBEN ESTAR AL PRINCIPIO) ---
def f_m(valor):
    """Convierte números a formato moneda $ 1.000"""
    try:
...
def cl(texto):
    """Limpia el formato moneda para volver a número"""
    try:
        return int(str(texto).replace(".", "").replace("$", "").replace(" ", ""))
    except:
        return 0

# --- 1. CONFIGURACIÓN DE PÁGINA Y ESTILO (SideBar Blanco) ---
st.set_page_config(page_title="NL GESTIÓN", layout="wide")

st.markdown("""
    <style>
        [data-testid="stSidebar"] { background-color: white !important; }
        [data-testid="stSidebar"] .stRadio div label p { color: black !important; }
    </style>
    """, unsafe_allow_html=True)

# --- 2. MOTOR DE CONEXIÓN (Definido primero para evitar NameError) ---
# --- CONEXIÓN VELOZ CON CACHE ---
# --- MOTOR DE DATOS REFORZADO ---
@st.cache_resource
def obtener_engine():
    c = st.secrets["database"]
    # Agregamos 'options' para acelerar la escritura
    url = f"postgresql://{c['user']}:{c['password']}@{c['host']}:{c['port']}/{c['database']}?sslmode=require"
    return create_engine(url, isolation_level="AUTOCOMMIT")

def db_query(query, params=(), commit=False):
    engine = obtener_engine()
    try:
        # Traductor de ? a %s y limpieza de corchetes
        query = query.replace('?', '%s').replace('[', '"').replace(']', '"')
        
        with engine.connect() as conn:
            if commit:
                # Inyectamos RETURNING para recuperar IDs de contratos/pagos
        ...
        if "already exists" not in str(e):
            st.error(f"⚠️ Error en DB: {e}")
        return None

# --- 3. INICIALIZACIÓN DE TABLAS (Postgres Clean) ---
def mantenimiento_db():
    tablas = [
        "CREATE TABLE IF NOT EXISTS bloques (id SERIAL PRIMARY KEY, nombre TEXT, direccion TEXT, barrio TEXT, localidad TEXT)",
        "CREATE TABLE IF NOT EXISTS inquilinos (id SERIAL PRIMARY KEY, nombre TEXT, dni TEXT, celular TEXT, procedencia TEXT, grupo TEXT, em_nombre TEXT, em_tel TEXT, emergencia TEXT)",
        "CREATE TABLE IF NOT EXISTS inmuebles (id SERIAL PRIMARY KEY, id_bloque INTEGER, tipo TEXT, precio_alquiler BIGINT DEFAULT 0, costo_contrato BIGINT DEFAULT 0, deposito_base BIGINT DEFAULT 0, estado TEXT DEFAULT 'Libre')",
        # Tabla Contratos reforzada
        "CREATE TABLE IF NOT EXISTS contratos (id SERIAL PRIMARY KEY, id_inmueble INTEGER, id_inquilino INTEGER, fecha_inicio DATE, fecha_fin DATE, meses INTEGER, monto_alquiler BIGINT DEFAULT 0, monto_contrato BIGINT DEFAULT 0, monto_deposito BIGINT DEFAULT 0, activo INTEGER DEFAULT 1)",
        # Tabla Deudas reforzada (Aquí saltó tu error de mes_anio y fecha_cobro)
        "CREATE TABLE IF NOT EXISTS deudas (id SERIAL PRIMARY KEY, id_contrato INTEGER, concepto TEXT, mes_anio TEXT, monto_debe BIGINT DEFAULT 0, monto_pago BIGINT DEFAULT 0, pagado INTEGER DEFAULT 0, fecha_cobro DATE, fecha_pago DATE)",
        "CREATE TABLE IF NOT EXISTS gestiones (id SERIAL PRIMARY KEY, fecha DATE, propiedad TEXT, locatario TEXT, detalle TEXT, monto NUMERIC, estado TEXT)",
        "CREATE TABLE IF NOT EXISTS lotes (id SERIAL PRIMARY KEY, id_desarrollo INTEGER, numero_lote TEXT, manzana TEXT, superficie REAL, precio_contado REAL, precio_financiado REAL, estado TEXT DEFAULT 'Libre')"
    ]
    for sql in tablas:
        db_query(sql, commit=True)

# Solo ejecutamos mantenimiento si la app ya cargó el motor
mantenimiento_db()

# --- 4. MENÚ Y VARIABLES ---
if 'pdf_listo' not in st.session_state: st.session_state['pdf_listo'] = None
if 'id_listo' not in st.session_state: st.session_state['id_listo'] = None

with st.sidebar:
    st.title("NL GESTIÓN")
    # Devolvemos "Cargas" y tus otras opciones originales
    menu = st.radio("MENÚ:", [
        "🏠 Inventario", 
        "📝 Nuevo Contrato", 
        "💰 Cobranzas", 
    ...
    st.divider()
    if st.button("🔍 Verificar Datos Reales"):
        # En Postgres, a veces el count(*) falla si la tabla no está entre comillas dobles
        res = db_query('SELECT COUNT(*) FROM "inmuebles"')
        if res is not None:
            # res es un DataFrame, sacamos el valor de la fila 0, columna 0
            cantidad = res.iloc[0,0]
            st.sidebar.write(f"Registros encontrados: {cantidad}")
            
# A PARTIR DE AQUÍ CONTINÚA TU LÓGICA DE 1500 LÍNEAS
...
if menu == "🏠 Inventario":
    ...
    # luego sacar
    # --- DEBUG PROFESIONAL ---
    # --- 1. CAPA DE DEBUG (Para ver si hay datos) ---
    res_u_debug = db_query('SELECT * FROM inmuebles')
    res_b_debug = db_query('SELECT * FROM bloques')

    # Shape[0] nos da la cantidad de filas sin errores de Python
    c_u = res_u_debug.shape[0] if res_u_debug is not None else 0
    c_b = res_b_debug.shape[0] if res_b_debug is not None else 0

    st.write(f"📊 **Estado de Base de Datos:** {c_u} Unidades | {c_b} Edificios")

    # --- 2. CONSULTA DE INVENTARIO ---
    q_inv = """
        SELECT 
            COALESCE(b.nombre, 'Sin Edificio') as "Inmueble", 
            i.tipo as "Unidad", 
            i.precio_alquiler as "Alquiler", 
            CASE WHEN c.activo = 1 THEN '🔴 OCUPADO' ELSE '🟢 LIBRE' END as "Estado",
            CASE 
                WHEN c.activo = 1 THEN CAST(c.fecha_fin AS TEXT)
                ELSE 'DISPONIBLE HOY' 
    ...
    if sel_inmueble != "Todos":
        q_inv += f" AND b.nombre = '{sel_inmueble}'"

    df_inv = db_query(q_inv)

    # --- 3. VISUALIZACIÓN (Alineación corregida para evitar SyntaxError) ---
    if df_inv is not None and not df_inv.empty:
        ...
        # Este else está ahora perfectamente alineado con el 'if df_inv is not None'
        st.warning("⚠️ No se encontraron unidades para mostrar.")
        ...
        # --- FORMATEO PROTEGIDO (Evita el KeyError) ---
        if df_u_show is not None and not df_u_show.empty:
         for col in ['Alquiler', 'Contrato', 'Deposito']:
             if col in df_u_show.columns:
                 df_u_show[col] = df_u_show[col].apply(f_m)
             elif col.lower() in df_u_show.columns: # Por si Postgres la pasó a minúsculas
                 df_u_show[col.lower()] = df_u_show[col.lower()].apply(f_m)
    ...
    with t1:
        ...
        if df_inm is not None and not df_inm.empty:
            ...
            with t2:
                ...
                # --- 1. CONSULTA PARA EL SELECTOR DE GESTIÓN ---
                df_u_gestion = db_query("""
                    SELECT 
                        i.id, 
                ...
                if df_u_gestion is not None and not df_u_gestion.empty:
                    # Mostramos la tabla resumen para que el usuario vea qué hay
                    st.write("### Unidades Registradas")
                    
                    # Formateamos el precio para la tabla
                    df_u_show = df_u_gestion.copy()
                    if 'Alquiler' in df_u_show.columns:
                        df_u_show['Alquiler'] = df_u_show['Alquiler'].apply(f_m)
                    
                    st.dataframe(df_u_show[['Inmueble', 'Unidad', 'Alquiler', 'Estado']], use_container_width=True, hide_index=True)

                    # El Selector que estaba fallando
                    lista_opciones = df_u_gestion.index.tolist()
                    sel_idx = st.selectbox(
                        "Seleccione Unidad para editar", 
                        lista_opciones,
                        format_func=lambda x: f"{df_u_gestion.loc[x, 'Inmueble']} - {df_u_gestion.loc[x, 'Unidad']}"
                    )
                    
                    # Variable clave para el resto del formulario
                    id_unidad_seleccionada = df_u_gestion.loc[sel_idx, 'id']

                else:
                    st.warning("⚠️ No se encontraron unidades en la base de datos. Verifique la carga inicial.")

                # --- FIN DEL BLOQUE DE VISUALIZACIÓN ---
                
                st.write("---")
                # --- BLOQUE DE GESTIÓN DE UNIDADES CORREGIDO ---
                # --- BLOQUE DE GESTIÓN PROTEGIDO ---
                # 1. Definimos las variables al inicio para que EXISTAN SI O SI
                dat_u = {} 
                u_nombre = ""
                u_precio = 0
                u_contrato = 0
                ...
                df_uni_gest = db_query("""
                    SELECT i.id, b.nombre as "Inmueble", i.tipo as "Unidad" 
                    FROM inmuebles i 
                    JOIN bloques b ON i.id_bloque = b.id
                """)

                if df_uni_gest is not None and not df_uni_gest.empty:
                    df_uni_gest = df_uni_gest.reset_index(drop=True)
                    sel_u_idx = st.selectbox(
                        "Seleccione Unidad para gestionar", 
                        df_uni_gest.index.tolist(), 
                        format_func=lambda x: f"{df_uni_gest.loc[x, 'Inmueble']} - {df_uni_gest.loc[x, 'Unidad']}"
                    )
                    
                    id_u_sel = df_uni_gest.loc[sel_u_idx, 'id']
                    
                    # 2. Traemos los datos de la unidad
                    res_u = db_query("SELECT * FROM inmuebles WHERE id = %s", (id_u_sel,))
                    
                    if res_u is not None and not res_u.empty:
                        dat_u = res_u.iloc[0].to_dict()
                        
                        # 3. Extraemos los valores (Aquí es donde daba el error 936, ahora es seguro)
                        u_nombre = dat_u.get('tipo') or dat_u.get('Unidad') or ""
                        u_precio = dat_u.get('precio_alquiler') or dat_u.get('Alquiler') or 0
                        u_contrato = dat_u.get('costo_contrato') or dat_u.get('Contrato') or 0
                        u_deposito = dat_u.get('deposito_base') or dat_u.get('Deposito') or 0
                        u_estado = dat_u.get('estado', 'Libre')

                        st.subheader(f"Editando: {u_nombre}")
                        
                        with st.form("form_edit_u"):
                            eu_t = st.text_input("Descripción / Unidad", value=str(u_nombre))
                            eu_a = st.number_input("Precio Alquiler", value=int(u_precio))
                            eu_c = st.number_input("Costo Contrato", value=int(u_contrato))
                            eu_d = st.number_input("Depósito Base", value=int(u_deposito))
                            
                            lista_est = ["Libre", "Ocupado", "Reservado"]
                            idx_e = lista_est.index(u_estado) if u_estado in lista_est else 0
                            eu_e = st.selectbox("Estado", lista_est, index=idx_e)
                            
                            if st.form_submit_button("💾 Guardar Cambios"):
                                sql_upd = """
                                    UPDATE inmuebles 
                                    SET tipo = %s, precio_alquiler = %s, costo_contrato = %s, deposito_base = %s, estado = %s 
                                    WHERE id = %s
                                """
                                db_query(sql_upd, (eu_t, eu_a, eu_c, eu_d, eu_e, id_u_sel), commit=True)
                                st.success("✅ Unidad actualizada")
                                st.rerun()
                else:
                    st.info("No hay unidades disponibles. Realice la restauración de datos.")
    
                with st.form("f_u_edit"):
                    c1, c2, c3 = st.columns(3)
                    # --- REEMPLAZO FINAL PARA GESTIÓN DE UNIDADES ---
                    # Estas líneas aseguran que los datos existan bajo cualquier nombre
                    u_nombre = dat_u.get('Unidad', dat_u.get('tipo', ''))
                    u_precio = dat_u.get('Alquiler', dat_u.get('precio_alquiler', 0))
                    u_costo  = dat_u.get('Contrato', dat_u.get('costo_contrato', 0))
                    u_depo   = dat_u.get('Deposito', dat_u.get('deposito_base', 0))

                    # Ahora tus inputs usan estas variables 'seguras'
                    eu_t = st.text_input("Descripción", value=str(u_nombre))
                    eu_a = st.number_input("Precio Alquiler", value=int(u_precio))
                    eu_c = st.number_input("Costo Contrato", value=int(u_costo))
                    eu_d = st.number_input("Depósito Base", value=int(u_depo))
...
# --- 4. CONTRATOS (V.9.5 - RENOVACIÓN CON CONTINUIDAD Y PDF) ---
    with t4:
        ...
        # 2. CAPA DE CONTROL (Cláusula de Guarda Senior)
        if df_cont is None or df_cont.empty:
            st.info("ℹ️ La base de datos de contratos está vacía. Proceda con la Restauración de Backup en la sección correspondiente.")
            # IMPORTANTE: Aquí termina la ejecución de esta pestaña si no hay datos
        else:
            # 3. SOLO SI HAY DATOS, EJECUTAMOS EL RESTO
            df_display = df_cont[['ID_Contrato', 'Inmueble', 'Unidad', 'Inquilino', 'Inicio', 'Fin', 'Alq_Actual']].copy()
            st.dataframe(df_display, use_container_width=True)
            
            lista_c = df_cont['ID_Contrato'].tolist()
            sel_c_id = st.selectbox("Seleccione ID de Contrato para ver detalle", lista_c)
            
            # Filtramos de forma segura
            datos_especificos = df_cont[df_cont['ID_Contrato'] == sel_c_id]
            
            if not datos_especificos.empty:
                # AQUÍ SE CREA LA VARIABLE SEGURA
                row_sel = datos_especificos.iloc[0]
                
                st.write(f"### Detalle: {row_sel.get('Inquilino', 'Sin nombre')}")
                
                # --- AQUÍ ADENTRO DEBES MOVER TODA TU LÓGICA DE DETALLE (BOTONES, PDF, ETC.) ---
                # Si tenés líneas que usan 'row_sel' más abajo, metelas ACÁ ADENTRO.
                
            else:
                st.warning("No se encontraron registros para el ID seleccionado.")
                
        # --- 3. SECCIÓN DE RESTAURACIÓN (Siempre disponible) ---
        # Aquí debe ir tu código de st.file_uploader y el procesamiento del Excel
        # Al estar fuera del 'if' anterior, el botón siempre será visible y accesible.
    ...
    with t5:
        ...
        with c_imp:
            ...
            archivo = st.file_uploader("Seleccionar .xlsx", type=["xlsx"], key="rest_v3")
            if archivo and st.button("🚀 Iniciar Carga de Datos"):
                try:
                    engine = obtener_engine()
                    ...
                    # Mapeo exacto
                    mapping = {
                        'Inmuebles': 'bloques',   # <-- Esto llena la tabla 'bloques' (id, nombre...)
                        'Unidades': 'inmuebles',  # <-- Esto llena 'inmuebles' (id_bloque, tipo...)
                        'Inquilinos': 'inquilinos',
                        'Contratos': 'contratos',
                        'Deudas': 'deudas'
                    }
                    
                    container = st.empty() # Para mensajes en tiempo real
                    
                    for hoja, tabla in mapping.items():
                        if hoja in dfs:
                            container.write(f"⏳ Procesando: {hoja}...")
                            # Limpieza agresiva
                            db_query(f'TRUNCATE TABLE "{tabla}" RESTART IDENTITY CASCADE;', commit=True)
                            
                            # Carga directa
                            data = dfs[hoja]
                            # Limpiamos nombres de columnas por si tienen espacios/puntos del Excel
                            data.columns = [c.strip() for c in data.columns]
                            
                            data.to_sql(tabla, engine, if_exists='append', index=False, method='multi', chunksize=100)
                            container.write(f"✅ {hoja} cargado.")
                    
                    st.success("¡TODO CARGADO!")
                    st.balloons()
                    st.rerun()
                except Exception as e:
                    st.error(f"❌ Falló en {hoja}: {e}")
    ...
    with t6:
        ...
        """)
        # --- PROTECCIÓN DE VISUALIZACIÓN DE SALDOS ---
        if df_alq is not None and not df_alq.empty:
            df_v = df_alq.copy()
            
            # Si la columna 'Saldo' no existe, la calculamos nosotros
            if 'Saldo' not in df_v.columns:
                # Intentamos calcularla restando debe y pago (ajustá los nombres si son distintos)
                debe = df_v.get('monto_debe', 0)
                pago = df_v.get('monto_pago', 0)
                df_v['Saldo'] = debe - pago

            # Aplicamos el formato solo si tenemos la columna
            df_v['Saldo_Visual'] = df_v['Saldo'].apply(lambda x: f"🔴 {f_m(x)}" if x > 0 else "🟢 Al día")
            st.dataframe(df_v)
...
# ===================================================
# 2. ESTO ES NUEVO NO SE QUE FUNCION TENDRA
# ===================================================
elif menu == "⚙️ Sistema":
    ...
    with col1:
        st.subheader("Restaurar desde Excel")
        archivo = st.file_uploader("Subir Backup", type=["xlsx"])
        if archivo and st.button("🚀 Iniciar Restauración"):
            try:
                engine = obtener_engine()
                dfs = pd.read_excel(archivo, sheet_name=None)
                mapping = {
                    ...
                    'Unidades': 'inmuebles', 
                    'Inquilinos': 'inquilinos', 
                    'Contratos': 'contratos',  # <-- Esta es la que falló recién
                    'Deudas': 'deudas'
                }
    ...
    with col2:
        ...
        if st.button("🗑️ VACIAR TODO"):
            db_query("TRUNCATE TABLE bloques, inmuebles, inquilinos, contratos, deudas RESTART IDENTITY CASCADE;", commit=True)
            st.warning("Base de datos limpia.")
            st.rerun()

# FINAL DEL ARCHIVO
st.sidebar.info(f"Conectado a: Supabase Cloud")
