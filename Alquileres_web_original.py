...
if menu == "🏠 Inventario":
    ...
    if df_inv is not None and not df_inv.empty:
        ...
        # Reemplazamos applymap por map (o usamos el formateo directo de Streamlit)
        try:
            # Opción A: Usando el nuevo estándar de Pandas
            st.dataframe(df_show.style.map(color_estado, subset=['Estado']), use_container_width=True, hide_index=True)
        except AttributeError:
            # Opción B: Retrocompatibilidad para versiones viejas si falla la A
            st.dataframe(df_show.style.applymap(color_estado, subset=['Estado']), use_container_width=True, hide_index=True)
    else:
    ...
    if u_raw is not None and not u_raw.empty and i_raw is not None and not i_raw.empty:
        ...
        with st.form("f_v72"):
            ...
            if st.form_submit_button("✅ GUARDAR Y GENERAR"):
                try:
                    f_v = fini + timedelta(days=meses * 30)
